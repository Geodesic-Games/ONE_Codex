---
name: one
description: Use when the user wants to inspect or manage ONE boards, columns, items, comments, project planning, Procurement, Remote Machines, administration audits, owner-managed users and permissions, private item files, Hardware assets and documents, People documents, Finance records and workflows, protected backups, publish branded HTML/PDF client decks, or intentionally retrieve their organization's ONE-published branding. The remote MCP server enforces the connected person's current permissions through OAuth or a time-limited Member MCP key.
---
...
5. For images or video, upload bounded base64 content with `upload_deck_asset` and the exact current `expected_revision`; the returned deck revision becomes authoritative for the next write. Reference the returned asset ID in the draft. Use `set_deck_access` only when the user explicitly requests public or passcode access. For passcode mode, give the user the returned one-time browser setup link; never ask for, accept, repeat, retrieve, or log the passcode in the AI conversation.
...
7. After publishing, verify the returned public and PDF URLs. Use `unpublish_deck` only after explicit confirmation and pass the exact current published revision ID; explain that it removes public access while preserving the editable draft and immutable revision history.
...
6. The audit omits API-key hashes, reusable secrets, bridge settings, connector tokens, invitation acceptance secrets, and cloud credentials. Never ask another tool for those values. API-key creation/revocation, bridge and retention settings, and client install/update/uninstall are intentionally not plugin tools; direct an authorized person to the reviewed ONE workflow. For owner-authorized user and permission changes, use the workspace access workflow below.

## Workspace users and permissions

- Use the signed-in workspace owner's OAuth connection for `get_workspace_access`, `invite_workspace_user`, `update_workspace_user_access`, and `save_workspace_permission_group`. The server verifies the designated owner's active account and verified sign-in on every operation. Administrators, members, guests, and all API keys are denied. Never retry an access denial through another credential or a generic database tool.
- These tools belong to the `operations` family. If the connection uses a reduced tool list, inspect `list_tool_families` and enable `operations` with `set_tool_families`, preserving the other selected families. Family selection does not grant permissions.
- Read `get_workspace_access` first to resolve the exact email, current role, group IDs, area IDs, module IDs, and revision. Use its email filter for a particular existing account and respect truncation flags. Show the intended person, areas and access levels when they are not already explicit in the user's request. Carry out authorized changes without asking for the same approval again.
- Add a new person with `invite_workspace_user`. It creates an email-bound Google-account invitation and returns a link; it does not send email or activate an account. Share the link with the requester. After the person accepts, read the account and assign access. Do not invent a temporary password or claim an email was sent.
- Use `update_workspace_user_access` for role, active/disabled status, personal-request scope, group membership, direct grants, and individual BackOffice sections. `permission_group_ids` replaces the membership list; `direct_access` and `backoffice_modules` patch only the listed areas. Preserve unrelated assignments. `none` removes a direct grant; a stronger group grant still applies. Moving an administrator to scoped access requires an explicit Member role change. Owner accounts and project guests cannot be changed with these tools.
- Use `save_workspace_permission_group` to create a group or update a returned group ID. Its board list replaces the group's areas for every assigned member, including an empty list. BackOffice sections require direct grants from the owner and cannot be added to a group. Explain the effect on existing members before a group-wide change unless the request already specifies it.
- Pass the fresh `expected_revision` and exact `confirm_email` or `confirm_name` with every mutation. Re-read after success or an uncertain response. On a revision conflict, inspect the changes before retrying. Never overwrite a newer permission decision automatically.
- Independent Project Tracker access also uses owner-only OAuth for `set_project_tracker_access`; read `get_project_tracker_access` first and use its own current revision. Production board access and Project Tracker access are separate grants.
