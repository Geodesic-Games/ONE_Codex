...
The same single plugin can create, review, passcode-protect, publish, and unpublish branded client decks at `https://decks.geotech.one/<slug>`, including a matching downloadable PDF. Content-only deck updates use the hosted ONE API and do not require a repository checkout, GitHub action, or Firebase deployment.
