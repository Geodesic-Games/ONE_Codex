...
assert.match(skill, /Passcodes are write-only secrets/);
...
assert.match(pluginReadme, /manually configured user-level `ONE` or `one` MCP connection/);
...
assert.match(rootReadme, /manually configured user-level `ONE` or `one` MCP connection/);
